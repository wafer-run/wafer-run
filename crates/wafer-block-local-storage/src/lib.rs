//! Local filesystem storage block — `wafer-run/local-storage`.
//!
...
impl Block for LocalStorageBlock {
    fn info(&self) -> BlockInfo {
        BlockInfo {
            name: "wafer-run/local-storage".to_string(),
    ...
    async fn handle(&self, _ctx: &dyn Context, msg: &mut Message) -> Result_ {
        let service = self
            .service
            .get()
            .expect("wafer-run/local-storage: not initialized — call lifecycle(Init) first");
    ...
    async fn lifecycle(
        ...
        if event.event_type == LifecycleType::Init && self.service.get().is_none() {
            ...
            let svc = LocalStorageService::new(&root)
                .map_err(|e| WaferError::new("init", format!("wafer-run/local-storage: {}", e)))?;
...
pub fn register(w: &mut wafer_run::Wafer) {
    w.register_block("wafer-run/local-storage", Arc::new(LocalStorageBlock::new()));
}
