//! PostgreSQL database block — `@wafer/postgres`.
//!
...
impl Block for PostgresDatabaseBlock {
    ...
    async fn handle(&self, _ctx: &dyn Context, msg: &mut Message) -> Result_ {
        let service = self
            .service
            .get()
            .expect("@wafer/postgres: not initialized — call lifecycle(Init) first");
    ...
    async fn lifecycle(
        ...
        if event.event_type == LifecycleType::Init && self.service.get().is_none() {
            ...
            let url = crate::blocks::env_or_config_str("DATABASE_URL", config.as_ref(), "url")
                .ok_or_else(|| {
                    WaferError::new(
                        "config",
                        "@wafer/postgres: requires DATABASE_URL env var or url config",
                    )
                })?;

            let svc = PostgresDatabaseService::connect(&url)
                .await
                .map_err(|e| WaferError::new("init", format!("@wafer/postgres: {}", e)))?;
...
pub fn register(w: &mut wafer_run::Wafer) {
    w.register_block("@wafer/postgres", Arc::new(PostgresDatabaseBlock::new()));
}
