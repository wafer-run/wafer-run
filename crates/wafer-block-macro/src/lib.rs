...
use syn::{
    parse_macro_input, Expr, ExprArray, ExprLit, Ident, ImplItem, ImplItemFn, ItemImpl, Lit, Token,
};
...
/// - `export_macro` — name of the local export macro (e.g. `"export_block"`).
///   When set, the macro generates code for a standalone WASM block that uses
///   locally generated WIT bindings (via `wit_bindgen::generate!` with type
///   remapping). The Guest trait and export macro are referenced from the local
///   crate instead of from `wafer_block`.
///
/// # Example (standalone WASM block)
///
/// ```rust,ignore
/// // Step 1: Generate local WIT bindings, remapping types to wafer_block
...
/// struct MyBlock;
///
/// // Step 2: Use the macro with export_macro pointing to the local macro
/// #[wafer_block(
///     name = "my-block",
///     version = "0.1.0",
...
pub fn wafer_block(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let export_macro = args.get_str("export_macro");
    ...
    // Generate code depending on whether this is a standalone WASM block or not.
    let expanded = if let Some(macro_name) = export_macro {
        // Standalone WASM block: use local WIT bindings (generated by the
        // block's own wit_bindgen::generate! call with type remapping).
        // The Guest trait comes from the local `exports` module.
        // The export macro is the one named in the attribute.
        let export_macro_ident = Ident::new(&macro_name, proc_macro::Span::call_site().into());

        quote! {
            #other_impl

            impl crate::exports::wafer::block_world::block::Guest for #struct_ty {
            ...
            #export_macro_ident!(#struct_ty);
        }
    } else {
        // Inline / native block: use wafer_block's types and export macro directly.
        quote! {
            #other_impl
