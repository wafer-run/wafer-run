//! SQLite database block — `wafer-run/sqlite`.
//!
...
impl Block for SqliteDatabaseBlock {
    fn info(&self) -> BlockInfo {
        BlockInfo {
            name: "wafer-run/sqlite".to_string(),
    ...
    async fn handle(&self, _ctx: &dyn Context, msg: &mut Message) -> Result_ {
        let service = self
            .service
            .get()
            .expect("wafer-run/sqlite: not initialized — call lifecycle(Init) first");
    ...
    async fn lifecycle(
        ...
        if event.event_type == LifecycleType::Init && self.service.get().is_none() {
            ...
            let svc = SQLiteDatabaseService::open(&path)
                .map_err(|e| WaferError::new("init", format!("wafer-run/sqlite: {}", e)))?;
...
pub fn register(w: &mut wafer_run::Wafer) {
    w.register_block("wafer-run/sqlite", Arc::new(SqliteDatabaseBlock::new()));
}
