...
async fn loads_declared_keys_present_in_map() {
    ...
    data.insert("MY_ORG__AUTH__JWT_SECRET".to_string(), "secret".to_string());
    let src = StaticConfigSource::new(data);

    let declared = vec![key("MY_ORG__AUTH__JWT_SECRET", "", true)];
    let cfg: EnvBlockConfig = src
        .load_for_block("my-org/auth", &declared)
        .await
        .expect("load_for_block should succeed");

    assert_eq!(cfg.get("MY_ORG__AUTH__JWT_SECRET"), Some("secret"));
}
...
async fn missing_required_key_returns_config_error() {
    ...
    match err {
        ConfigError::MissingRequired { block, key } => {
            assert_eq!(block, "my-org/auth");
            assert_eq!(key, "MY_ORG__AUTH__JWT_SECRET");
        }
...
async fn missing_optional_key_returns_default() {
    let src = StaticConfigSource::new(HashMap::new());
    let declared = vec![key("MY_ORG__AUTH__SESSION_TTL", "3600", false)];
    ...
    assert_eq!(cfg.get("MY_ORG__AUTH__SESSION_TTL"), Some("3600"));
}
...
async fn ignores_keys_not_declared_by_block() {
    let mut data = HashMap::new();
    data.insert("MY_ORG__AUTH__JWT_SECRET".to_string(), "secret".to_string());
    data.insert("MY_ORG__OTHER__KEY".to_string(), "v".to_string());
    let src = StaticConfigSource::new(data);
    ...
    assert_eq!(cfg.get("MY_ORG__AUTH__JWT_SECRET"), Some("secret"));
    assert_eq!(cfg.get("MY_ORG__OTHER__KEY"), None);
}

#[tokio::test]
async fn optional_key_no_default_absent_returns_none() {
    let src = StaticConfigSource::new(HashMap::new());
    let declared = vec![key("MY_ORG__AUTH__OPTIONAL_TOKEN", "", false)];
    ...
    assert_eq!(cfg.get("MY_ORG__AUTH__OPTIONAL_TOKEN"), None);
}
