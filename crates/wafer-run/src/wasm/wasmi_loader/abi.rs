...
pub(super) struct WasmiHostState {
    ...
    /// The host-owned meta of the message this guest was invoked with — the
    /// identity (`auth.*`, SEC-01) established upstream by the trusted host /
    /// auth middleware, and the inbound sensitive headers. Seeded per call;
    /// applied to any nested `call_block` the guest initiates via the
    /// streaming ABI, so the guest cannot forge identity for a callee.
    pub(super) host_owned_meta: super::meta::HostOwnedMeta,
...
mod tests {
    ...
    fn host_state_with_table_cap(max_table_elements: u32) -> WasmiHostState {
        WasmiHostState {
            ...
            host_owned_meta: crate::wasm::wasmi_loader::meta::HostOwnedMeta::default(),
