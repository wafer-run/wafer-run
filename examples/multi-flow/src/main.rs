...
async fn main() {
    ...
    wafer.add_flow_json(r#"{
        ...
        "steps": [
            { "id": "validate", "block": "validate-signup", "input": { "email": "$.input.email", "name": "$.input.name" } },
            { "id": "create-account", "block": "account-creator", "input": { "email": "$.validate.email", "name": "$.validate.name" } },
            { "id": "assign-role", "block": "role-assigner", "input": { "user_id": "$.create-account.user_id", "plan": "$.input.plan" },
              "next": [
                { "when": "$.input.plan == 'enterprise'", "step": "enterprise-setup" },
                { "step": "send-welcome" }
              ]
            },
            { "id": "enterprise-setup", "block": "enterprise-provisioner", "input": { "user_id": "$.create-account.user_id" } },
            { "id": "send-welcome", "block": "email-sender", "input": { "to": "$.validate.email", "user_id": "$.create-account.user_id", "template": "welcome" } }
        ],
    ...
    wafer.add_flow_json(r#"{
        ...
        "steps": [
            { "id": "load-prefs", "block": "preference-loader", "input": { "user_id": "$.input.user_id" } },
            { "id": "route", "block": "notification-router", "input": { "prefs": "$.load-prefs.preferences", "message": "$.input.message" },
              "next": [
            ...
            { "id": "send-email", "block": "email-sender", "input": { "to": "$.load-prefs.preferences.email", "body": "$.input.message" } },
            { "id": "send-push", "block": "push-sender", "input": { "device_token": "$.load-prefs.preferences.device_token", "body": "$.input.message" } },
            { "id": "send-webhook", "block": "webhook-caller", "input": { "url": "$.load-prefs.preferences.webhook_url", "payload": "$.input.message" } }
        ],
    ...
    wafer
        .register_block("greeter", Arc::new(GreeterBlock))
        .expect("register greeter");
    wafer
        .register_block("health", Arc::new(HealthBlock))
        .expect("register health");
    wafer
        .register_block("not-found", Arc::new(NotFoundBlock))
