...
- Config variables use the `ConfigVar` type declared in `wafer-block/src/types/config_var.rs`. Blocks declare their own vars in `BlockInfo::config_keys`. Validation rules are derived from naming conventions and `input_type`, not hardcoded lists.
